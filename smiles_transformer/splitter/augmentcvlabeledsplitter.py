...
class AugmentCVLabeledSplitter(BaseSplitterFactory):
    """Splits data into training, testing, and evaluation sets using a split column.
    This splitter is for cross validation WITH augmented data,
    to make sure that the augmented molecules are in the same splits.
    ...
    def labeler(
        ...
        """Assigns labels to data for train, test, and evaluation splits.

        Args:
            X (pd.DataFrame): Data to be labeled.
            y (pd.Series, optional): Target labels. Defaults to None.
            test_size (float, optional): Proportion for test split. Defaults to 0.25.
        ...
        data["split"] = "train"
        data.iloc[:test_size, data.columns.get_loc("split")] = "test"
        data.iloc[
            test_size : (test_size + eval_size),  # noqa E203
            data.columns.get_loc("split"),
        ] = "eval"
